...
#define NO_WORD 0
#define WORD_EXIST 1
#define LIMIT 30 // количество элементов

void menu()
{
    printf("\t\t\tМеню\n\n");
    printf("\t1) Прочитать деревa и хеш-таблицу из файла.\n\
        2) Вывести дерева и хеш-таблицу на экран.\n\
...
{
    FILE *f_open = fopen(file_name, "r"); // входный файл
    hash_t *temp_hash_table = NULL; // вспомогательная структура хеш-таблицы 
    ...
    if (f_open)
    {
        ...
        if (temp_hash_table)
            while (!feof(f_open))
            {
                ...
                temp_hash_table = load_table(temp_hash_table, word, LIMIT);
...
void search_word(tree_node_t **tree, tree_node_t **balanced_tree, hash_t **hash_table)
{
    ...
    if (flag == WORD_EXIST)
    {
        ...
        flag = lookup_word_hash(*hash_table, word);
